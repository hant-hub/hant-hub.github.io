    ...
    */
    ...
    function CreateMultiVertBuffer(ctx, prog) {
        ...
        for (var i = 0; i < prog.attrs.length; i++) {
            ...
            gl.bindBuffer(gl.ARRAY_BUFFER, buffer);
            gl.vertexAttribPointer(attr.location, attr.count, attr.type, false, attr.size, 0);
            gl.enableVertexAttribArray(attr.location);
    ...
    function BindTexture(ctx, uniform, prog, loc, flat, texture) {
        var gl = ctx.gl;
    ...
    function SetUniform(ctx, prog, index, value) {
        ...
        switch (info.type) {
            ...
            default: {
                console.log("Unknown Uniform Type! " + info.type);
                return;
            }
        }
    ...
    function UploadMultiVertBuffer(ctx, buffer, verts) {
        var gl = ctx.gl;
        ...
        for (var i = 0; i < verts.length; i++) {
            gl.bindBuffer(gl.ARRAY_BUFFER, buffer.buffers[i]);
            gl.bufferData(gl.ARRAY_BUFFER, Float32Array.from(verts[i]), gl.DYNAMIC_DRAW);
        }

        buffer.size = verts[0].length / buffer.div;
        buffer.cap = buffer.size;
    }
    ...
    function ResizeMultiVertBuffer(ctx, buffer, newsize) {
        var gl = ctx.gl;
        ...
        for (var i = 0; i < buffer.buffers.length; i++) {
            gl.bindBuffer(gl.ARRAY_BUFFER, buffer.buffers[i]);
            gl.bufferData(gl.ARRAY_BUFFER, newsize * buffer.vert_sizes[i], gl.DYNAMIC_DRAW);
        }
    ...
    function PushMultiVerts(ctx, buffer, verts) {
        var gl = ctx.gl;
        ...
        if (buffer.size + (verts[0].length/buffer.div) > buffer.cap) {
            return 1;
        }
        ...
        for (var i = 0; i < verts.length; i++) {
            ...
            gl.bufferSubData(
                gl.ARRAY_BUFFER, 
                buffer.size * buffer.vert_sizes[i],
                Float32Array.from(verts[i]),
            );
        }
    ...
    function Upload3DData(ctx, tex, data, width, height, depth) {
        var gl = ctx.gl;
    ...
    return {
        ...
        Create3DTex: Create3DTex,
        Upload3DData: Upload3DData,
