...
function UpdateBoids(ctx, camera, boid_buf, boids) {
    ...
    for (var i = flip; i < boids.length; i += 2) {
        ...
        for (var j = 0; j < boids.length; j++) {
            var op = new Vector3().set(boids[j].pos);
            var ov = new Vector3().set(boids[j].vel);
        ...
        target.mul(1/boids.length);
        target.sub(pos);
        target.mul(0.05);
        vel.add(target);


        var curr = valueNoise(pos.elements[0], pos.elements[1], pos.elements[2]);
        var next = valueNoise(
            pos.elements[0] + vel.elements[0],
            pos.elements[1] + vel.elements[1],
            pos.elements[2] + vel.elements[2]);

        if (next >= 0.5) {
            var turn = Vector3.cross(vel, new Vector3([vel.elements[1], -vel.elements[0], vel.elements[2]]));
            turn.mul(-1);
            turn.normalize();
            vel.add(turn);
        }
